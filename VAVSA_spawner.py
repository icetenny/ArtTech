        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                if ui_is_active:
                    ...
                    client_receiver = CustomSocket(host, port_receiver)
                    client_receiver.clientConnect()
                    client_receiver.sendMsg(client_receiver.sock, json.dumps(cam_out))
                    rev_data = json.loads(client_receiver.recvMsg(client_receiver.sock))
                    client_receiver.clientDisconnect()
                    show_ghost.change_image_from_array(img=np.array(rev_data["img"], dtype="uint8"))
